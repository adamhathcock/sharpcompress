...
public class ZipReaderTests : ReaderTests
{
  ...
  [Fact]
  public void Zip_Deflate_Streamed2_Read() => Read("Zip.deflate.dd-.zip", CompressionType.Deflate);
  ...
  public void Zip_LZMA_WinzipAES_Read() =>
    ...
    {
      using (
        Stream stream = File.OpenRead(Path.Combine(TEST_ARCHIVES_PATH, "Zip.lzma.WinzipAES.zip"))
      )
      using (var reader = ZipReader.Open(stream, new ReaderOptions { Password = "test" }))
      {
        while (reader.MoveToNextEntry())
  ...
  {
    using (
      Stream stream = File.OpenRead(Path.Combine(TEST_ARCHIVES_PATH, "Zip.deflate.WinzipAES.zip"))
    )
    using (var reader = ZipReader.Open(stream, new ReaderOptions { Password = "test" }))
    {
      while (reader.MoveToNextEntry())

...
public class ZipArchiveTests : ArchiveTests
{
  ...
  [Fact]
  public void Zip_Deflate_Streamed2_ArchiveStreamRead() => ArchiveStreamRead("Zip.deflate.dd-.zip");
  ...
  //will detect and load other files
  [Fact]
  public void Zip_Deflate_Split_ArchiveFirstFileRead() => ArchiveFileRead("Zip.deflate.split.001");
  ...
  {
    foreach (
      var file in Directory.EnumerateFiles(ORIGINAL_FILES_PATH, "*.*", SearchOption.AllDirectories)
    )
    {
  ...
  public void Zip_Forced_Ignores_UnicodePathExtra()
  {
    ...
    using (var stream = File.OpenRead(zipPath))
    {
      var archive = ArchiveFactory.Open(
        ...
        new ReaderOptions
        {
          ArchiveEncoding = new ArchiveEncoding { Default = Encoding.GetEncoding("shift_jis") },
        }
      );
      var reader = archive.ExtractAllEntries();
    ...
    using (var stream = File.OpenRead(zipPath))
    {
      var archive = ArchiveFactory.Open(
        ...
        new ReaderOptions
        {
          ArchiveEncoding = new ArchiveEncoding { Forced = Encoding.GetEncoding("shift_jis") },
        }
      );
      var reader = archive.ExtractAllEntries();
      reader.MoveToNextEntry();
  ...
  {
    using var archive = ZipArchive.Open(Path.Combine(TEST_ARCHIVES_PATH, "Zip.EntryComment.zip"));
    var firstEntry = archive.Entries.First();

  ...
  private void DoRar_Multi_Reader(string[] archives)
  {
    using (
      var reader = RarReader.Open(
        archives.Select(s => Path.Combine(TEST_ARCHIVES_PATH, s)).Select(p => File.OpenRead(p))
      )
    )
    {
    ...
    using (var reader = ReaderFactory.Open(stream))
    {
      while (reader.MoveToNextEntry())
      {
        if (!reader.Entry.IsDirectory)
        {
          ...
          var folder = Path.GetDirectoryName(reader.Entry.Key) ?? throw new ArgumentNullException();

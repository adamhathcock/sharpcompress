...
public class GZipReaderAsyncTests : ReaderTests
{
    ...
    public async ValueTask GZip_Reader_Generic2_Async()
    {
        ...
        await using var reader = ReaderFactory.OpenAsyncArchive(new AsyncOnlyStream(stream));
    ...
    )
    {
        ...
        await using (var reader = ReaderFactory.OpenAsyncArchive(testStream, options, default))
        {

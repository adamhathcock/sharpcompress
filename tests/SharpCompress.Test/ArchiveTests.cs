...
public class ArchiveTests : ReaderTests
{
  ...
  {
    using (
      var archive = ArchiveFactory.Open(testArchives.Select(a => new FileInfo(a)), readerOptions)
    )
    {
      foreach (var entry in archive.Entries.Where(entry => !entry.IsDirectory))
      {
  ...
  {
    using (
      var archive = ArchiveFactory.Open(testArchives.Select(f => new FileInfo(f)), readerOptions)
    )
    {
      foreach (var entry in archive.Entries.Where(entry => !entry.IsDirectory))
      {

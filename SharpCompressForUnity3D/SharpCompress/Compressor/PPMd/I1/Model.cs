    ...
    internal class Model
    {
        ...
        Label_01CC:
            if (num != 0)
            {
                ...
                if (context.NumberStatistics != 0)
                {
                    ...
                    firstStateFrequency = (byte) (1 + (((2 * num8) <= num9) ? (uint)(((5 * num8) > num9) ? 1 : 0) : (((num8 + (2 * num9)) - 3) / num9)));
                }
                else
                {
        ...
        private PpmContext CutOff(int order, PpmContext context)
        {
            ...
            if (context.NumberStatistics == 0)
            {
                ...
                if ((Pointer)firstState.Successor >= this.Allocator.BaseUnit)
                {
            ...
            for (firstState = context.Statistics + numberStatistics; firstState >= context.Statistics; firstState++ )
            {
        ...
        private void DecodeBinarySymbol(PpmContext context)
        {
            ...
            int index1 = this.probabilities[firstState.Frequency - 1];
            int index2 = ((this.numberStatisticsToBinarySummaryIndex[context.Suffix.NumberStatistics] + this.previousSuccess) + context.Flags) + ((this.runLength >> 0x1a) & 0x20);
            if (this.Coder.RangeGetCurrentShiftCount(14) < this.binarySummary[index1, index2])
            {
                ...
                this.Coder.HighCount = this.binarySummary[index1, index2];
                //ushort num1 = this.binarySummary[num, i];
                //num1[0] = (ushort) (num1[0] + ((ushort) (0x80L - Mean(this.binarySummary[num, i], 7, 2))));
            ...
            {
                this.Coder.LowCount = this.binarySummary[index1, index2];
                //ushort escapeFrequency = this.binarySummary[index1, index2];
                //escapeFrequency[0] = (ushort) (escapeFrequency[0] - ((ushort) Mean(this.binarySummary[index1, index2], 7, 2)));
                binarySummary[index1, index2] -= (ushort)Mean(binarySummary[index1, index2], PeriodBitCount, 2);
                this.Coder.HighCount = 0x4000;
                this.initialEscape = ExponentialEscapes[this.binarySummary[index1, index2] >> 10];
        ...
        internal int DecodeBlock(Stream source, byte[] buffer, int offset, int count)
        {
            ...
            while (num < count)
            {
                ...
                offset++;
                num++;
                if ((this.orderFall == 0) && ((Pointer)this.foundState.Successor >= this.Allocator.BaseUnit))
                {
        ...
        private void DecodeSymbol1(PpmContext context)
        {
            ...
            uint count = this.Coder.RangeGetCurrentCount();
            if (count < highCount)
            {
                this.Coder.HighCount = highCount;
                this.previousSuccess = ((2 * this.Coder.HighCount) >= this.Coder.Scale) ? ((byte) 1) : ((byte) 0);
                this.foundState = statistics;
                highCount += 4;
                this.foundState.Frequency = (byte) highCount;
        ...
        Label_0193:
            ...
            if ((highCount += (++statistics).Frequency) <= count)
            {
                if (--numberStatistics == 0)
                {
                    this.Coder.LowCount = highCount;
        ...
        private void EncodeBinarySymbol(int symbol, PpmContext context)
        {
            ...
            if (firstState.Symbol == symbol)
            {
                ...
                this.Coder.LowCount = 0;
                this.Coder.HighCount = this.binarySummary[index1, index2];
                //ushort num1 = this.binarySummary[index1, index2];
                //num1[0] = (ushort) (num1[0] + ((ushort) (0x80L - Mean(this.binarySummary[index1, index2], 7, 2))));
                binarySummary[index1, index2] +=
                   (ushort)(Interval - Mean(binarySummary[index1, index2], PeriodBitCount, 2));
                this.previousSuccess = 1;
                this.runLength++;
        ...
        {
            See2Context see2Context = this.MakeEscapeFrequency(context);
            uint num2 = 0;
            uint index = (uint) (context.NumberStatistics - this.numberMasked);
            PpmState state = context.Statistics - 1;
            do
            {
                uint num;
            ...
            this.Coder.HighCount = this.Coder.Scale;
            see2Context.Summary = (ushort) (see2Context.Summary + ((ushort) this.Coder.Scale));
            this.numberMasked = context.NumberStatistics;
        }
        ...
        private void Refresh(uint oldUnitCount, bool scale, PpmContext context)
        {
            ...
            int escapeFrequency = context.SummaryFrequency - statistics.Frequency;
            ...
            while (--numberStatistics != 0);
            escapeFrequency = (escapeFrequency + num3) >> num3;
            context.SummaryFrequency = (ushort) (context.SummaryFrequency + ((ushort) escapeFrequency));
        }
        ...
        private PpmContext RemoveBinaryContexts(int order, PpmContext context)
        {
            ...
            {
                firstState = context.FirstState;
                if (((Pointer)firstState.Successor >= this.Allocator.BaseUnit) && (order < this.modelOrder))
                {
            ...
            for (firstState = context.Statistics + context.NumberStatistics; firstState >= context.Statistics; firstState--)
            {
                if (((Pointer)firstState.Successor >= this.Allocator.BaseUnit) && (order < this.modelOrder))
                {
                    firstState.Successor = this.RemoveBinaryContexts(order + 1, firstState.Successor);
        ...
        private void Rescale(PpmContext context)
        {
            ...
            context.SummaryFrequency = (ushort) (context.SummaryFrequency + 4);
            uint escapeFrequency = (uint) (context.SummaryFrequency - foundState.Frequency);  
            ...
            do
            {
                ...
                escapeFrequency -= state3.Frequency;
                foundState.Frequency = (byte) ((foundState.Frequency + num2) >> 1);
            ...
            if (foundState.Frequency == 0)
            {
                ...
                escapeFrequency += numberStatistics;
                uint unitCount = (uint) ((context.NumberStatistics + 2) >> 1);
                ...
                if (context.NumberStatistics == 0)
                {
                    ...
                    frequency = (byte) ((((2 * frequency) + escapeFrequency) - 1) /escapeFrequency);
                    if (frequency > 0x29)
                    {
                        frequency = 0x29;
                    }
        ...
        private void RestoreModel(PpmContext context, PpmContext minimumContext, PpmContext foundStateSuccessor)
        {
            ...
            if (this.method > ModelRestorationMethod.Freeze)
            {
                ...
                this.Allocator.GlueCount += (uint)(((this.Allocator.MemoryNodes[1].Stamp & 1) == 0) ? 1 : 0);
            }
        ...
        private void UpdateModel(PpmContext minimumContext)
        {
            ...
            if ((frequency < 0x1f) && (suffix != PpmContext.Zero))
            {
                if (suffix.NumberStatistics != 0)
                {
                    ...
                    num3 = (uint)((zero.Frequency < 0x73) ? 2 : 0);

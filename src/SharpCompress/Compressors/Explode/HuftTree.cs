  ...
  decoded. */
  {
    ...
    for (; numberOfBitsInCurrentCode <= maximumCodeLength; numberOfBitsInCurrentCode++)
    {
      ...
      while ((counterForCodes--) != 0)
      {
        ...
        while (
          numberOfBitsInCurrentCode > bitsBeforeThisTable + arrLX[stackOfBitsPerTable + tableLevel]
        )
        {
          ...
          int fBitCounter1 = 1 << (counter = numberOfBitsInCurrentCode - bitsBeforeThisTable);
        ...
        {
          vHuft1.NumberOfExtraBits = (arrValuesInOrderOfBitLength[pIndex] < 256 ? 32 : 31); /* 256 is end-of-block code */
        ...
        while ((counterCurrentCode & ((1 << bitsBeforeThisTable) - 1)) != bitOffset[tableLevel])
        {

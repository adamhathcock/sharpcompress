    ...
    while (true)
    {
      ...
      switch (cmd.OpCode)
      {
        ...
        case VMCommands.VM_ADD:
          {
            ...
            var result = (int)(
              ((value1 + (long)GetValue(cmd.IsByteMode, Mem, op2))) & unchecked((int)0xffffffff)
            );
            if (cmd.IsByteMode)
            {
              result &= 0xff;
        ...
        case VMCommands.VM_XOR:
          {
            var result = GetValue(cmd.IsByteMode, Mem, op1) ^ GetValue(cmd.IsByteMode, Mem, op2);
            flags = (VMFlags)(result == 0 ? (int)VMFlags.VM_FZ : result & (int)VMFlags.VM_FS);
            SetValue(cmd.IsByteMode, Mem, op1, result);
          }
          break;
        ...
        case VMCommands.VM_NEG:
          {
            ...
            flags = (VMFlags)(
              result == 0 ? (int)VMFlags.VM_FZ : (int)VMFlags.VM_FC | (result & (int)VMFlags.VM_FS)
            );
            SetValue(cmd.IsByteMode, Mem, op1, result);
          }
          break;
      ...
      case VMStandardFilters.VMSF_E8E9:
        {
          ...
          var cmpByte2 = (byte)((filterType == VMStandardFilters.VMSF_E8E9) ? 0xe9 : 0xe8);
            ...
            if (Byte >= 0)
            {
              ...
              if (cmdMask != 0)
              {
                for (var i = 0; i <= 2; i++)
                {
                  ...
                  {
                    var startPos = (i * 41) + 5;
                    var opType = filterItanium_GetBits(curPos, startPos + 37, 4);
                    if (opType == 5)
                    {
                      var offset = filterItanium_GetBits(curPos, startPos + 13, 20);

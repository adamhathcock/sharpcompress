...
public sealed class LZipStream : Stream, IStreamStack
{
  ...
  public LZipStream(Stream stream, CompressionMode mode)
  {
    ...
    else
    {
      ...
      _stream = new Crc32Stream(
        new LzmaStream(new LzmaEncoderProperties(true, dSize), false, _countingWritableSubStream)
      );
#if DEBUG_STREAMS
  ...
  private static readonly byte[] headerBytes = [(byte)'L', (byte)'Z', (byte)'I', (byte)'P', 1, 113];

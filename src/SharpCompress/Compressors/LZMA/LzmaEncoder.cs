...
internal class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
{
  ...
  private class LenEncoder
  {
    ...
    public void SetPrices(uint posState, uint numSymbols, uint[] prices, uint st)
    {
      ...
      {
        prices[st + i] =
          b1 + _highCoder.GetPrice(i - Base.K_NUM_LOW_LEN_SYMBOLS - Base.K_NUM_MID_LEN_SYMBOLS);
      }
    }
  }
  ...
  private uint GetOptimum(uint position, out uint backRes)
  {
    ...
    while (true)
    {
      ...
      if (matchByte == currentByte && !(nextOptimum._posPrev < cur && nextOptimum._backPrev == 0))
      {
      ...
      if (!nextIsChar && matchByte != currentByte)
      {
        ...
        if (lenTest2 >= 2)
        {
          ...
          var nextRepMatchPrice =
            curAnd1Price
            + _isMatch[(state2._index << Base.K_NUM_POS_STATES_BITS_MAX) + posStateNext].GetPrice1()
          ...
          if (lenTest2 >= 2)
          {
            ...
            var curAndLenCharPrice =
              ...
              + _isMatch[(state2._index << Base.K_NUM_POS_STATES_BITS_MAX) + posStateNext]
                .GetPrice0()
              + _literalEncoder
                .GetSubCoder(position + lenTest, _matchFinder.GetIndexByte((int)lenTest - 1 - 1))
                .GetPrice(
                  true,
    ...
    for (uint lenToPosState = 0; lenToPosState < Base.K_NUM_LEN_TO_POS_STATES; lenToPosState++)
    {
      ...
      for (posSlot = Base.K_END_POS_MODEL_INDEX; posSlot < _distTableSize; posSlot++)
      {
        _posSlotPrices[st + posSlot] += (
          (((posSlot >> 1) - 1) - Base.K_NUM_ALIGN_BITS) << BitEncoder.K_NUM_BIT_PRICE_SHIFT_BITS
        );
      }

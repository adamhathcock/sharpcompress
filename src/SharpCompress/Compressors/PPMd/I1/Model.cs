...
internal partial class Model
{
  ...
  private void UpdateModel(PpmContext minimumContext)
  {
    ...
    for (; currentContext != minimumContext; currentContext = currentContext.Suffix)
    {
      ...
      {
        cf = (uint)(
          4 + ((cf > 9 * sf) ? 1 : 0) + ((cf > 12 * sf) ? 1 : 0) + ((cf > 15 * sf) ? 1 : 0)
        );
    ...
    do
    {
      ...
      else
      {
        ...
        state.Frequency += (byte)(
          ((context.Suffix.NumberStatistics == 0) ? 1 : 0) & ((state.Frequency < 24) ? 1 : 0)
        );
      }
  ...
  private static int Mean(int sum, int shift, int round) => (sum + (1 << (shift - round))) >> shift;
